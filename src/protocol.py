...
class Protocol:
    ...
    def register_task(self, bus, task, buffer_size):
        self._tasks.append((bus, task, buffer_size))
    ...
    def assemble(self, sensor_period):
        ...
        start_char = 1
        # after that start character, we follow up with the of the total message.
        # It's the length in one byte, followd by the XOR of that length.
        # The reason for this verified storage is that we need a reliable
        # way of syncing after the start-char to the expected number of bytes
        message_length = 2
        ...
        payload_size = sum(buffer_size for _, _, buffer_size in self._tasks)
        # the final checksum is just all bytes so far summed up as unsigned bytes
        checksum_size = 1
        ...
        descriptor_byte_count_offset = start_char + message_length + name_length
        ...
        if payload_start % 4:
            ...
            buffer_size += padding
        print(payload_start, buffer_size)
        ...
        descriptor = memoryview(self.buffer)[descriptor_start:descriptor_start+descriptor_length]
        newjoy.init(sensor_period, self.buffer)
        task_byte_offset = payload_start
        for i, (bus, task, task_size) in enumerate(self._tasks):
            offset = i // 2
            current = descriptor[offset]
            current |= task << (4 * i % 2)
            descriptor[offset] = current
            newjoy.add_task(bus, task, task_byte_offset)
